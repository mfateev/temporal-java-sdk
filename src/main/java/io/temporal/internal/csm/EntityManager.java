...
public final class EntityManager {
  ...
  private final Map<Long, EntityStateMachine> commands = new HashMap<>();
  ...
  public final void handleEvent(HistoryEvent event) {
    if (
    /*isReplaying() && */ isCommandEvent(event)) {
      handleCommand(event);
      return;
    }
    Long initialCommandEventId = getInitialCommandEventId(event);
    EntityStateMachine c = commands.get(initialCommandEventId);
    if (c != null) {
      c.handleEvent(event);
      if (c.isFinalState()) {
  ...
  private void prepareCommands() {
    ...
    // activity completion. And code execution can lead to cancellation of any of the commands.
    // So a second pass that doesn't lead to code execution is needed to filter out cancelled
    // commands.
    List<NewCommand> firstPass = new ArrayList<>(newCommands.size());
    while (true) {
      ...
      firstPass.add(newCommand);
    }
    preparedCommands = new ArrayList<>(firstPass.size());
    for (NewCommand newCommand : firstPass) {
      if (!newCommand.isCanceled()) {
        System.out.println(
            "prepareCommands canceled=" + newCommand.isCanceled() + ": " + newCommand.getCommand());
  ...
  private class WorkflowTaskCommandsListener implements WorkflowTaskStateMachine.Listener {
    ...
    public void workflowTaskStarted(long startedEventId, long currentTimeMillis) {
      ...
      if (EntityManager.this.workflowTaskStartedEventId == startedEventId) {
        prepareCommands();
      }
    }
