...
/**
 * Registry for Kotlin activity implementations used in testing.
 *
 * This class maintains a mapping of activity type names to their implementations,
 * supporting both regular and suspend activity methods. It enables runtime registration
 * of activity mocks in tests via [KTestWorkflowExtension].
 *
 * This registry is designed specifically for testing scenarios where activity implementations
 * (including mocks) need to be registered at runtime, after the worker has started.
 */
internal class KTestActivityRegistry {
     ...
     */
    @Volatile
    var dynamicActivityFallback: KDynamicActivity? = null
     ...
     */
    fun registerMockImplementation(mockImplementation: Any) {
     ...
     */
    fun hasActivity(activityType: String): Boolean = activities.containsKey(activityType)

    /**
     * Get all registered activity type names.
     */
    fun getRegisteredTypes(): Set<String> = activities.keys.toSet()
     ...
     */
    fun execute(activityType: String, args: KEncodedValues): Any? {
