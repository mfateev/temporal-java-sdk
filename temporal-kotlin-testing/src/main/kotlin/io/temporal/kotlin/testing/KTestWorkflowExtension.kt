 ...
 * - Injecting workflow stubs, clients, and environments into test methods
 ...
 *     fun `test workflow with specific initial time`(workflow: MyWorkflow) {
...
public class KTestWorkflowExtension private constructor(
    ...
    // ========== Commit 17: Workflow interface type support ==========

    /**
     * Set of parameter types that can be resolved by this extension.
     * Includes core types and workflow interfaces extracted from registered implementations.
     */
    private val supportedParameterTypes: Set<Class<*>> = run {
        val types = mutableSetOf<Class<*>>()

        // Core types
        types.add(KTestWorkflowEnvironment::class.java)
        types.add(KWorkflowClient::class.java)
        types.add(KWorkflowOptions::class.java)
        types.add(KWorker::class.java)
        types.add(Worker::class.java)

        // Extract workflow interfaces from registered implementations
        config.workflowTypes.keys.forEach { workflowType ->
            if (!DynamicWorkflow::class.java.isAssignableFrom(workflowType)) {
                try {
                    ...
                    metadata.workflowInterfaces.forEach { interfaceMetadata ->
                        types.add(interfaceMetadata.interfaceClass)
                    }
                } catch (e: Exception) {
    ...
    /**
     * Whether any registered workflow type implements DynamicWorkflow.
     * If true, we need to check for workflow interfaces dynamically.
     */
    private val includesDynamicWorkflow: Boolean = config.workflowTypes.keys.any {
        DynamicWorkflow::class.java.isAssignableFrom(it)
    }

    // ========== Commit 16: Parameter resolution ==========
    ...
    override fun supportsParameter(
        ...
        // If we have dynamic workflows, check if the parameter type is a valid workflow interface
        if (!includesDynamicWorkflow) return false

        // Try to validate as a workflow interface
        return try {
    ...
    override fun resolveParameter(
        ...
        return when (parameterType) {
            ...
            Worker::class.java -> getKWorker(store).worker
                ...
                // Create workflow stub for the interface type
 ...
 *
 *     @Test
 *     fun `test workflow`(workflow: MyWorkflow) {
 *         val result = workflow.execute("input")
 *         assertEquals("expected", result)
 *     }
 * }
